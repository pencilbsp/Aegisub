...
- (void)setItem:(NSToolbarItem*)item forItemIdentifier:(NSString*)itemIdentifier;
- (void)removeItemForItemIdentifier:(NSString*)itemIdentifier;
...
- (void)setItem:(NSToolbarItem*)item forItemIdentifier:(NSString*)itemIdentifier
{
    if ( itemIdentifier && item )
        [m_items setObject:item forKey:itemIdentifier];
}

- (void)removeItemForItemIdentifier:(NSString*)itemIdentifier
{
    if ( itemIdentifier )
        [m_items removeObjectForKey:itemIdentifier];
}
...
{
    wxUnusedVar(toolbar);
    NSToolbarItem* item = [m_items objectForKey:itemIdentifier];
    if ( item && [item isKindOfClass:[wxNSToolbarItem class]] )
    {
        wxToolBarTool* tool = [(wxNSToolbarItem*)item implementation];
        if ( flag && tool && tool->IsControl() )
        {
        ...
        case wxTOOL_STYLE_BUTTON:
            {
                ...
                if (m_macToolbar != NULL)
                {
                    ...
                    [(wxNSToolbarDelegate*)[(NSToolbar*)m_macToolbar delegate] setItem:item forItemIdentifier:cfidentifier.AsNSString()];
                    tool->SetToolbarItemRef( item );
                }
...
bool wxToolBar::DoDeleteTool(size_t WXUNUSED(pos), wxToolBarToolBase *toolbase)
{
    ...
    if (m_macToolbar != NULL)
    {
        NSToolbarItem* item = tool->GetToolbarItemRef();
        if ( item )
        {
            [(wxNSToolbarDelegate*)[(NSToolbar*)m_macToolbar delegate] removeItemForItemIdentifier:[item itemIdentifier]];
        }
