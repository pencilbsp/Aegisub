...
@interface wxNSCenteredComboBoxCell : NSComboBoxCell
@end

@implementation wxNSCenteredComboBoxCell

- (NSRect)wx_centeredTitleRectForBounds:(NSRect)bounds
{
...
- (NSRect)wx_centeredTextFramePreservingWidth:(NSRect)frame
{
...
{
    return [self wx_centeredTitleRectForBounds:bounds];
}

- (void)drawInteriorWithFrame:(NSRect)cellFrame inView:(NSView *)controlView
{
    [super drawInteriorWithFrame:[self wx_centeredTextFramePreservingWidth:cellFrame] inView:controlView];
}

- (void)editWithFrame:(NSRect)rect inView:(NSView *)controlView editor:(NSText *)textObj delegate:(id)delegate event:(NSEvent *)event
{
    [super editWithFrame:[self wx_centeredTextFramePreservingWidth:rect] inView:controlView editor:textObj delegate:delegate event:event];
}

- (void)selectWithFrame:(NSRect)rect inView:(NSView *)controlView editor:(NSText *)textObj delegate:(id)delegate start:(NSInteger)selStart length:(NSInteger)selLength
{
    [super selectWithFrame:[self wx_centeredTextFramePreservingWidth:rect] inView:controlView editor:textObj delegate:delegate start:selStart length:selLength];
}

@end

static void wxOSXCenterComboBoxCellText(NSComboBox* combo)
{
    NSCell* cell = [combo cell];
    if ( [cell isKindOfClass:[NSComboBoxCell class]] &&
        ![cell isKindOfClass:[wxNSCenteredComboBoxCell class]] )
    {
        object_setClass(cell, [wxNSCenteredComboBoxCell class]);
    }
}
...
wxSize wxComboBox::DoGetBestSize() const
{
    ...
    
    {
        ...
        auto updateWidth = [&](const wxString& str)
        {
        ...
        updateWidth(GetValue());
        for (unsigned int i = 0; i < GetCount(); i++)
            updateWidth(GetString(i));
        
        // Add room for the popup arrow and Cocoa text field insets. Measuring
        // with wxClientDC gives the text width only, while NSComboBoxCell needs
        // a little more horizontal space or long values get clipped.
        lbWidth += 3 * lbHeight;
    }
    
    return wxSize(wxMax(lbWidth, baseSize.x), lbHeight);
}
