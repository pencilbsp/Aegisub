...
- (NSAttributedString *)attributedSubstringForProposedRange:(NSRange)aRange actualRange:(NSRangePointer)actualRange
{
    wxWidgetCocoaImpl* impl = (wxWidgetCocoaImpl* ) wxWidgetImpl::FindFromWXWidget( self );
    if (impl)
        return impl->attributedSubstringForProposedRange(aRange, actualRange);
    return nil;
}
...
- (NSArray*)validAttributesForMarkedText
{
    ...
    // but returning nil causes issues with some IMEs
    return @[];
}
...
- (NSRect)firstRectForCharacterRange:(NSRange)aRange actualRange:(NSRangePointer)actualRange
{
    wxWidgetCocoaImpl* impl = (wxWidgetCocoaImpl* ) wxWidgetImpl::FindFromWXWidget( self );
    if (impl)
        return impl->firstRectForCharacterRange(aRange, actualRange);
...
- (NSUInteger)characterIndexForPoint:(NSPoint)aPoint
{
    wxWidgetCocoaImpl* impl = (wxWidgetCocoaImpl* ) wxWidgetImpl::FindFromWXWidget( self );
    if (impl)
        return impl->characterIndexForPoint(aPoint);
    return NSNotFound;
}
...
{
    wxString wxText = wxCFStringRef::AsString(text);
    wxString currentVal;
    if (GetWXPeer()) {
