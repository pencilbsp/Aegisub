...
public :
    ...
    virtual NSRange             markedRange() const;
    virtual NSRange             selectedRange() const;
    virtual NSRect              firstRectForCharacterRange(NSRange aRange, NSRangePointer actualRange);
    virtual NSAttributedString* attributedSubstringForProposedRange(NSRange aRange, NSRangePointer actualRange);
    virtual NSUInteger          characterIndexForPoint(NSPoint aPoint);
    void                        ClearMarkedTextInEditor();
#endif // __OBJC__
...
#ifdef __OBJC__
    // IME (Input Method Editor) state for NSTextInputClient
    NSString* m_markedText;         // current composition string (retained)
